...
public class BRPhoneNumberFormatter implements TextWatcher {
	...
	private String formatUsNumber(Editable text) {
		...
		// Check if we are dealing with the new phone format, with an additional
		// digit
		...
		// (5 in case we are dealing with the new longer phone format: (xx)
		// xxxxx-xxxx
		if (totalDigitCount - alreadyPlacedDigitCount > dashAfter) {
			formattedString.append(allDigitString.substring(
					alreadyPlacedDigitCount, alreadyPlacedDigitCount
							+ dashAfter)
					+ "-");
